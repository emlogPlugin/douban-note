...
class DoubanOauth {
     ...
     */
    public $authorizationCode, $tokens, $accessToken, $refreshToken; //已修改，增加$refreshToken
     ...
     * @return string
     */
    public function access()  //已修改，返回值
    {
        // 获取Access_token请求链接
		...
		$this->expiresin = $this->tokens->expires_in;
		
		$this->doubanuserid = $this->tokens->douban_user_id;
     ...
     * @return string
     */
    public function refresh()  //已修改，新增加
    {
        // 获取Access_token请求链接
        ...
        // 使用curl模拟请求，获取token信息
        $this->tokens = $this->curl($refreshUrl, 'POST', $header);
     ...
     * @return string
     */
    public function authorizationUrl() //已修改熟悉protect为public
    {
	...
	/**
     * @brief 生成豆瓣refresh_token完整获取链接
     *
     * @return string
     */
    protected function refreshUrl() //已修改，新增加
    {
        return $this->accessUri.
            '?client_id='.$this->clientId.
            '&client_secret='.$this->secret.
            ...
            '&refresh_token='.$this->old_refresh_token;
    }
